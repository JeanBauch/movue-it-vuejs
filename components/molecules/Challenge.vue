...
export default Vue.extend<unknown, any, unknown, ChallengeType>({
	...
	methods: {
		...mapMutations({
			...
			setCurrentChallengeIndex: `Challenges/${ChallegesMT.SET_CURRENT_CHALLENGE_INDEX}`,
			completedChallenge: `Challenges/${ChallegesMT.COMPLETE_CHALLENGE}`,
		}),
		resetChallenges () {
			this.resetTime();
		...
		challengeSucceeded () {
			...
			this.completedChallenge(this.amount);
